    ...
    async car_new_arrival(job: Job, page: Page) {
        ...
        page.on("request", async req => {
            if (req.url().indexOf("/api/pc/feed/") > -1) {
                ...
                const feedUrlSigned = await page.evaluate(`
                (() => {
                    ...
                    return (${JSON.stringify(feedUrl)} + "&_signature=" + sig).replace("/toutiao/", "/");
                })()
                `);
                feedUrlR(feedUrlSigned);
                await req.abort();
            }
        });
        ...
        const str = res.body.toString("utf-8");
