...
class UserAgentsTask {
    ...
    async generateUserAgentsTs() {
        const userAgents = (await appInfo.db.findList("userAgents", {}, {_id: true}))
            .filter(item => item._id.startsWith("Mozilla/5.0 "))
            .map(item => item._id);
        return `
/*
...
const userAgents: string[] = ${JSON.stringify(userAgents, null, 4)};
